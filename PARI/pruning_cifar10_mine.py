...
def main():
    ...
    optimizer = torch.optim.Adam(net.parameters(), 1e-3, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
    
    if args.use_cuda:
        net.cuda()
        criterion.cuda()

    if args.use_pretrain:   #use pre-trained model
...
#    print("total remaining ratio is %f" % (args.rate_norm - args.rate_dist))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_acc_1, val_los_1 = validate(test_loader, net, criterion, log)
        if epoch % args.epoch_prune == 0 or epoch == args.epochs - 1:   # which epoch needs prune
            m.model = net
            m.if_zero()
            #current_rate_dist = args.rate_dist/ args.epochs * epoch     #prune rate increases linearly to the setup
...
class Mask:
    ...
    def get_filter_codebook(self, weight_torch, compress_rate, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            weight_vec = weight_torch.view(weight_torch.size()[0], -1)
            norm2 = torch.norm(weight_vec, 2, 1)    #calculate l2-norm
            norm2_np = norm2.cpu().numpy()
            filter_index = norm2_np.argsort()[:filter_pruned_num]   #get indexes
    ...
    def init_mask(self, rate_norm_per_layer, rate_dist_per_layer, dist_type):
        self.init_rate(rate_norm_per_layer, rate_dist_per_layer)    #get prune rate and GM rate for every layer

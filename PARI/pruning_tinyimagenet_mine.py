...
def main():
    ...
    val_filename = os.path.join(args.data, 'val/val_annotations.txt')
    ...
    for img , folder in val_img_dict.items():
        new_path = os.path.join(valdir, folder )
        if not os.path.exists(new_path):
            os.makedirs(new_path)
        
        if os.path.exists(os.path.join(valdir, img)):
            os.rename(os.path.join(valdir, img), os.path.join(new_path, img))
...
def train(train_loader, model, criterion, optimizer, epoch, log, m):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        losses.update(loss.item(), input.size(0))   ## 这里将作者的loss.data[0]修改成了loss.item()
...
class Mask:
    ...
    def get_pruned_index(self, weight_torch, prune_rate, weight_factor, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            print("filter codebook_mine done")
        else:
            pass
        return codebook
    ...
    def init_mask(self, rate_norm_per_layer, rate_dist_per_layer):
        ...
        for index, item in enumerate(self.model.parameters()):
            if index in self.mask_index:
                ...
                self.similar_matrix[index] = self.get_filter_similar(item.data, self.compress_rate[index],self.distance_rate[index],
                                                                     self.model_length[index])

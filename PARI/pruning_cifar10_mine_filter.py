...
def main():
    ...
    #net = torch.nn.DataParallel(net, device_ids=list(range(args.ngpu))) # use n gpu to train
    ...
    if args.use_pretrain:   #use pre-train model?
    ...
    m = Mask(net)   #generate mask
    m.init_length() #initialize the length of every mask
...
#    print("total remaining ratio is %f" % (args.rate_norm - args.rate_dist))
    ...
    #m.init_mask_mine(args.prune_rate, args.weight_factor) #need two parameters
    #    m.if_zero()
    #m.do_mask_mine()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_acc_1, val_los_1 = validate(test_loader, net, criterion, log)
        if epoch % args.epoch_prune == 0 or epoch == args.epochs - 1:   # get the epoch that needs prune
            m.model = net
            m.if_zero()
            #current_rate_dist = args.rate_dist/ args.epochs * epoch    #prune_rate increases linearly
            current_rate = args.prune_rate
            if epoch in write_epochs:
                m.init_mask_mine_v2(current_rate, args.weight_factor)
            else:
    ...
    with open('filters_visual_2.csv','w+') as myFile:      
        myWriter=csv.writer(myFile)  
        # write in row
        for index in range(16):
            myWriter.writerow(m.filters_selected[index])  
    log.close()
...
class Mask:
    ...
    def get_pruned_index_v2(self, weight_torch, prune_rate, weight_factor, length):
        codebook = np.ones(length)
        if len(weight_torch.size()) == 4:
            filter_pruned_num = int(weight_torch.size()[0] * prune_rate)
            weight_vec = weight_torch.view(weight_torch.size()[0], -1)
            ...
            filter_index = weight_sum.argsort()[:filter_pruned_num]
            self.filters_selected[self.counter] = filter_index
            kernel_length = weight_torch.size()[1] * weight_torch.size()[2] * weight_torch.size()[3]
            for x in range(0, len(filter_index)):
                codebook[filter_index[x] * kernel_length: (filter_index[x] + 1) * kernel_length] = 0
            print("filter codebook done")
    ...
    def get_filter_codebook(self, weight_torch, compress_rate, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            norm2_np = norm2.cpu().numpy()
            filter_index = norm2_np.argsort()[:filter_pruned_num]   #get filter indexses
    ...
    def init_mask(self, rate_norm_per_layer, rate_dist_per_layer, dist_type):
        self.init_rate(rate_norm_per_layer, rate_dist_per_layer)    #determine prune_rate and GM_rate
    ...
    def init_mask_mine_v2(self, per_prune_rate, per_weight_factor):
        self.init_rate_mine(per_prune_rate, per_weight_factor)
        print('prune_rate:\n {}'.format(self.prune_rate))
        print('weight_factor:\n {}'.format(self.weight_factor))
        for index, item in enumerate(self.model.parameters()):
            if index in self.mask_index:
                if index in [3, 27, 54, 81]:
                    self.matrix_mine[index] = self.get_pruned_index_v2(item.data, self.prune_rate[index], self.weight_factor[index],
                                                                    self.model_length[index])
                    self.matrix_mine[index] = self.convert2tensor(self.matrix_mine[index])
